...
export class TempTestProfiles {
     ...
     * globally
     */
    public static GLOBAL_INSTALL_NOTE: string = "\n\nNote: Make sure you have the current version of CA Brightside " +
    ...
    public static async createProfiles(testEnvironment: ITestEnvironment, profileTypes: string[] = []) {
        ...
        for (const type of profileTypes) {
            if (type === "db2") {
                profileNames.db2.push(await TempTestProfiles.createDb2Profile(testEnvironment));
            } else {
    ...
    /**
     * Helper to create a DB2 profile from test properties
     ...
     */
    private static async createDb2Profile(testEnvironment: ITestEnvironment) {
        const profileName: string = "tmp_db2_" + uuidv4();
        ...
        if (output.status !== 0 || output.stderr.toString().trim().length > 0) {
            throw new ImperativeError({
                msg: "Creation of Db2 profile '" + profileName + "' failed! You should delete the script: \n'" + scriptPath + "' " +
        ...
        this.log(testEnvironment, `Created Db2 profile '${profileName}'. Stdout from creation:\n${output.stdout.toString()}`);
        return profileName;
    }
    ...
    private static async deleteProfile(testEnvironment: ITestEnvironment, profileType: string, profileName: string) {
        ...
        const deleteProfileScript = TemporaryScripts.SHEBANG +
            `${TemporaryScripts.BRIGHT_BIN} profiles delete ${profileType} ${profileName} --force`;
